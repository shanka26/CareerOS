...
export function ResumeUpload() {
  ...
  return (
    ...
    >
      <label aria-disabled={pending} className="grid min-h-56 cursor-pointer place-items-center rounded-3xl border-2 border-dashed border-[var(--line)] bg-white/40 p-8 text-center transition hover:border-[var(--focus)] aria-disabled:cursor-wait aria-disabled:opacity-60">
        <input disabled={pending} className="sr-only" type="file" accept=".pdf,.doc,.docx,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={(event) => setFile(event.target.files?.[0])} />
        <span>
      ...
      {pending ? (
        <ProcessingIndicator
          title={`Processing ${file?.name ?? "your resume"}`}
          description="Uploading securely, reading visible text and scanned images, and preparing evidence-linked edits for your review. This may take a minute."
        />
      ) : null}
      {error ? <p role="alert" className="text-sm font-semibold text-red-700">{error}</p> : null}
      <Button type="submit" disabled={pending}>{pending ? <LoaderCircle className="mr-2 size-4 animate-spin" /> : null}{pending ? "Processing resume..." : "Analyze my resume with AI"}</Button>
