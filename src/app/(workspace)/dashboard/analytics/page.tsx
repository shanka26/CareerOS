import { calculateCareerMetrics, calculateSuggestionMetrics } from "@/domains/analytics/metrics";
...
export default async function AnalyticsPage() {
  ...
  const [applications, profile, suggestions] = await Promise.all([
    ...
    prisma.memorySuggestion.findMany({ where: { userId: session.user.id }, select: { status: true } }),
  ]);
  ...
  const suggestionMetrics = calculateSuggestionMetrics(suggestions.map(({ status }) => status));
  ...
  const cards = [
    ...
    ["Career Twin", `${completeness}%`, `${suggestionMetrics.pending} facts awaiting your review`],
    ["Suggestion acceptance", `${suggestionMetrics.acceptanceRate}%`, `${suggestionMetrics.accepted} of ${suggestionMetrics.reviewed} reviewed suggestions accepted`],
  ] as const;
