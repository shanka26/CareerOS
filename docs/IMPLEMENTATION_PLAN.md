...
- 2026-08-05: Separated the Jobs workspace into URL-addressable `My Jobs` and `Find Job` views. Personal/saved workspaces and manual import now live under the default `My Jobs` view; aggregated provider discovery and profile-derived search suggestions load only under `Find Job`. Added accessible navigation state and focused regression coverage.
