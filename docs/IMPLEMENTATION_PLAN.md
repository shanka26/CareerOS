...
- 2026-08-14: Expanded legitimate job-source coverage with optional native Jooble, Reed, and Careerjet adapters, including registered credential paths, bounded pagination, schema validation, salary normalization, publisher attribution, and Careerjet's required request metadata. Expanded one-click searches to Monster, CareerBuilder, SimplyHired, Built In, FlexJobs, Snagajob, Idealist, and Wellfound while retaining the existing LinkedIn, Indeed, Glassdoor, Google Jobs, ZipRecruiter, and Dice links. Restricted platforms remain outbound until approved partner search access is granted.
- 2026-08-14: Integrated everyday job platforms through compliant outbound searches. LinkedIn, Indeed, Glassdoor, Google Jobs, ZipRecruiter, and Dice now receive the active CareerOS keywords, location, and remote intent in one click; partner-restricted listings remain on their source platforms instead of being scraped or misrepresented as API results. Added URL-generation and UI regression coverage.
- 2026-08-14: Added Remote OK as another credential-free, hourly cached, directly attributed source and introduced source-diverse match ranking. Default results now interleave successful providers while preserving each provider's internal relevance order, preventing Arbeitnow or any other large feed from monopolizing the visible results.
- 2026-08-14: Added an approval-gated AI search coach to the Search Jobs workspace. The authenticated, no-store chat route grounds recommendations in verified Career Profile data and ephemeral user preferences, returns schema-validated keywords/location/workplace filters with a rationale, and runs nothing until the user applies the recommendation. Added API and component regression coverage.
- 2026-08-14: Expanded aggregated job discovery breadth and recall. Added Jobicy as a credential-free, hourly cached remote source; paginated Arbeitnow, Adzuna, and The Muse searches across three pages; increased USAJOBS result breadth; and replaced exact all-term matching with a tested minimum relevance threshold while preserving location and workplace filters.
- 2026-08-05: Separated the Jobs workspace into URL-addressable `My Jobs` and `Search Jobs` views. Aggregated provider discovery and profile-derived search suggestions now load under the default `Search Jobs` view so discovery is immediately visible; personal/saved workspaces and manual import remain available under `My Jobs`. Added accessible navigation state and focused regression coverage.
- 2026-08-04: Fixed resume-review validation clarity and a date-format mismatch exposed by real extracted data. Common resume dates such as `Jul 2020`, `07/2020`, and `2020/07` now normalize safely; invalid selected fields receive specific inline messages, an actionable error summary, accessible invalid state, and automatic focus/scroll to the first problem. Added component regression coverage for invalid-field navigation and successful date normalization.
...
- 2026-08-02: Activated OpenAI Responses API generation for user-facing resumes and cover letters. Requests use a verified-fact catalog, structured output, unknown-citation rejection, no-store mode, pseudonymous safety identifiers, immutable input snapshots, and exact provider/model/prompt provenance. A minimal live structured-output request passed with `gpt-5.6-terra` without sending career data.
